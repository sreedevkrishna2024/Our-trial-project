...
import ProgressTab from './Analysis/ProgressTab'
import ExportTab from './Tools/ExportTab'
import ImportTab from './Tools/ImportTab'
import SettingsTab from './Tools/SettingsTab'
import HelpTab from './Tools/HelpTab'
...
function OverviewTab({ 
  ...
  setGeneratedResults: React.Dispatch<React.SetStateAction<Array<{
    id: string
    content: string
    type: string
    timestamp: Date
  ...
  storyIdeasContext: string,
  setStoryIdeasContext: (value: string) => void,
  storyIdeasGenre: string,
  setStoryIdeasGenre: (value: string) => void
}) {
    ...
    <div className="overview-section space-y-8">
          ...
          <div className="space-y-4 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  value={storyIdeasContext}
                  onChange={(e) => setStoryIdeasContext(e.target.value)}
                  placeholder="e.g., time travel, space exploration..."
              ...
              <div>
                ...
                <select 
                  value={storyIdeasGenre}
                  onChange={(e) => setStoryIdeasGenre(e.target.value)}
                  className="input-professional w-full"
                >
        ...
        <div className="space-y-4">
          {storyIdeas.length > 0 ? (
            storyIdeas.map((idea, index) => (
              <div key={index} className="story-idea-bubble">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <p className="text-secondary-700 font-medium leading-relaxed">{idea}</p>
                    <div className="flex items-center gap-2 mt-3">
                      <span className="text-xs text-secondary-600 bg-white/60 px-3 py-1 rounded-full font-medium">
                        AI GENERATED
                      </span>
                  ...
                  <button 
                    ...
                    className="btn-secondary text-sm p-2 hover:scale-110 transition-transform duration-200"
                  >
      ...
      <ResultsDisplay 
        ...
        onRegenerate={(resultId) => {
          // Find the result and regenerate it
          const result = generatedResults.find(r => r.id === resultId)
          if (result) {
            if (result.type === 'story_idea') {
              generateStoryIdeas()
...
export function WritingDashboard() {
  ...
  const [storyIdeasContext, setStoryIdeasContext] = useState('')
  const [storyIdeasGenre, setStoryIdeasGenre] = useState('')
  const [characterWord, setCharacterWord] = useState('')
  const [characterDetails, setCharacterDetails] = useState('')
  const [characterGenre, setCharacterGenre] = useState('')
  ...
  const generateStoryIdeas = async () => {
    ...
    try {
      const prompt = storyIdeasContext.trim() 
        ? `Generate 5 creative and unique story ideas that are ALL directly related to "${storyIdeasContext}". Every single idea must incorporate "${storyIdeasContext}" as a central element. Each idea should be 1-2 sentences and include the genre. Format as a simple numbered list (1. Idea one, 2. Idea two, etc.).`
        : 'Generate 5 creative and unique story ideas. Each idea should be 1-2 sentences and include the genre. Format as a simple numbered list (1. Idea one, 2. Idea two, etc.).'

      console.log('Generating story ideas with prompt:', prompt)
      ...
      const response = await fetch('/api/ai/generate', {
        ...
        body: JSON.stringify({
          type: 'story_idea',
          prompt: prompt,
          genre: storyIdeasGenre || 'mixed',
          theme: storyIdeasContext || 'creative'
        })
      })

      const data = await response.json()
      console.log('Story ideas API response:', data)
      ...
      if (response.ok && data.success && data.data && data.data.content) {
        ...
        // Try to extract story ideas from different formats
        let ideas: string[] = []
        
        // Method 1: Look for numbered lists
        const numberedIdeas = content.split('\n')
          .filter((line: string) => line.trim().match(/^\d+\./))
          .map((line: string) => line.replace(/^\d+\.\s*/, '').trim())
          .filter((idea: string) => idea.length > 0)
        
        if (numberedIdeas.length > 0) {
          ideas = numberedIdeas
        } else {
          // Method 2: Look for JSON objects with titles
          try {
            const jsonMatch = content.match(/\{[\s\S]*"title"[\s\S]*?\}/g)
            if (jsonMatch) {
              ideas = jsonMatch.map((jsonStr: string, index: number) => {
                try {
                  const obj = JSON.parse(jsonStr)
                  return `${index + 1}. ${obj.title || obj.logline || 'Story Idea'}`
                } catch {
                  return `${index + 1}. Story Idea ${index + 1}`
                }
              })
            }
          } catch (e) {
        ...
        // Method 3: If no structured format found, create ideas from content
        if (ideas.length === 0) {
          const sentences = content.split(/[.!?]+/)
            .filter((s: string) => s.trim().length > 20)
            .slice(0, 5)
            .map((s: string, index: number) => `${index + 1}. ${s.trim()}`)
          ideas = sentences
        }
        ...
        // Handle API errors
        const errorMessage = data.error?.message || 'Failed to generate story ideas'
  ...
  // Generate AI character based on character type and details
  const generateCharacter = async () => {
    ...
    try {
      const response = await fetch('/api/ai/generate', {
        ...
        body: JSON.stringify({
          type: 'character',
          character1: characterWord,
          character2: characterDetails,
          genre: characterGenre || 'fantasy'
        })
      })
      ...
      if (response.ok && data.success && data.data && data.data.content) {
        ...
        const newResult = {
          ...
          type: 'character',
          timestamp: new Date(),
          prompt: `Create a detailed character based on "${characterWord}"${characterDetails ? ` with details: ${characterDetails}` : ''}`
        }
        setGeneratedResults(prev => [newResult, ...prev])
      } else {
        // Handle API errors
        const errorMessage = data.error?.message || 'Failed to generate character'
        console.error('API Error:', data.error)
        alert(`Error: ${errorMessage}`)
      }
    } catch (error) {
  ...
  const generateDialogue = async () => {
    if (!dialogueChar1.trim() || !dialogueChar2.trim()) {
    ...
    try {
      const response = await fetch('/api/ai/generate', {
        ...
        body: JSON.stringify({
          type: 'dialogue',
          prompt: `Write a natural dialogue between ${dialogueChar1} and ${dialogueChar2}. ${dialogueContext ? `Context: ${dialogueContext}` : ''} Make it engaging and reveal character through their speech patterns and word choices.`,
          character1: dialogueChar1,
          character2: dialogueChar2,
      ...
      if (response.ok && data.success && data.data && data.data.content) {
        setGeneratedDialogue(data.data.content)
        ...
        const newResult = {
          ...
          type: 'dialogue',
          timestamp: new Date(),
          prompt: `Write dialogue between ${dialogueChar1} and ${dialogueChar2}`
        }
        setGeneratedResults(prev => [newResult, ...prev])
      } else {
        const errorMessage = data.error?.message || 'Failed to generate dialogue'
        console.error('API Error:', data.error)
        alert(`Error: ${errorMessage}`)
      }
    } catch (error) {
  ...
  const generatePlot = async () => {
    ...
    try {
      const response = await fetch('/api/ai/generate', {
        ...
        body: JSON.stringify({
          type: 'plot',
          prompt: `Create a detailed plot outline for a ${plotGenre} story. ${plotTheme ? `Theme: ${plotTheme}` : ''} Include: beginning, middle, end, key plot points, conflicts, and character arcs. Make it engaging and well-structured.`,
          genre: plotGenre,
          theme: plotTheme
        })
      })
      ...
      if (response.ok && data.success && data.data && data.data.content) {
        setGeneratedPlot(data.data.content)
        ...
        const newResult = {
          ...
          type: 'plot',
          timestamp: new Date(),
          prompt: `Create plot outline for ${plotGenre} story`
        }
        setGeneratedResults(prev => [newResult, ...prev])
      } else {
        const errorMessage = data.error?.message || 'Failed to generate plot'
        console.error('API Error:', data.error)
        alert(`Error: ${errorMessage}`)
      }
    } catch (error) {
    ...
    try {
      const response = await fetch('/api/ai/generate', {
        ...
        body: JSON.stringify({
          type: 'world_building',
          prompt: worldName,
          genre: worldGenre || 'fantasy',
          setting: worldType || 'medieval',
          additionalDetails: worldDetails
        })
      })
      ...
      if (response.ok && data.success && data.data && data.data.content) {
        setGeneratedWorld(data.data.content)
        ...
        const newResult = {
          ...
          type: 'world_building',
          timestamp: new Date(),
          prompt: `Create world: ${worldName}`
        }
        setGeneratedResults(prev => [newResult, ...prev])
      } else {
        const errorMessage = data.error?.message || 'Failed to generate world'
        console.error('API Error:', data.error)
        alert(`Error: ${errorMessage}`)
      }
    } catch (error) {
    ...
    switch (activeTab) {
      case 'overview':
        return <OverviewTab 
          ...
          storyIdeasContext={storyIdeasContext}
          setStoryIdeasContext={setStoryIdeasContext}
          storyIdeasGenre={storyIdeasGenre}
          setStoryIdeasGenre={setStoryIdeasGenre}
        />
      case 'stories':
    ...
    <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g., vampire, wizard, detective, warrior..."
                className="input-professional w-full"
            ...
            <div>
              ...
              <select 
                ...
                onChange={(e) => setCharacterGenre(e.target.value)}
                className="input-professional w-full"
              >
                <option value="">Any Genre</option>
                <option value="fantasy">Fantasy</option>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="e.g., ancient vampire from medieval times, modern detective with psychic abilities, space warrior from Mars..."
          ...
          <LoadingButton
            ...
            loadingText="Generating Character..."
            disabled={!characterWord.trim()}
            className="w-full"
          >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      value={storyIdeasContext}
                      onChange={(e) => setStoryIdeasContext(e.target.value)}
                      placeholder="e.g., time travel, space exploration, mystery..."
                      className="input-professional w-full"
                    />
                  </div>
                  <div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <LoadingButton
                  ...
                  disabled={!dialogueChar1.trim() || !dialogueChar2.trim()}
                >
                ...
                {generatedDialogue && (
                  <div className="card p-6 bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="text-lg font-semibold text-blue-800">Generated Dialogue</h3>
                    </div>
                    <div className="prose prose-sm max-w-none">
                      <div className="whitespace-pre-wrap text-secondary-700 font-mono text-sm">
                        {generatedDialogue}
                      </div>
                    </div>
                    <div className="mt-4 flex gap-2">
                      <button
                        onClick={() => navigator.clipboard.writeText(generatedDialogue)}
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select 
                      value={plotGenre}
                      onChange={(e) => setPlotGenre(e.target.value)}
                      className="input-professional w-full"
                    >
                ...
                <LoadingButton
                  ...
                  loadingText="Building Plot..."
                  disabled={!plotGenre.trim()}
                >
                ...
                {generatedPlot && (
                  <div className="card p-6 bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="text-lg font-semibold text-purple-800">Generated Plot Outline</h3>
                    </div>
                    <div className="prose prose-sm max-w-none">
                      <div className="whitespace-pre-wrap text-secondary-700">
                        {generatedPlot}
                    ...
                    <div className="mt-4 flex gap-2">
                      <button
                        onClick={() => navigator.clipboard.writeText(generatedPlot)}
                        className="btn-secondary text-sm"
                      >
                        <Copy className="w-4 h-4" />
                        Copy
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g., floating islands, cyberpunk city, magical academy..."
                      className="input-professional w-full"
                    />
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g., magic system, technology level, political structure..."
                      className="input-professional w-full"
                    />
                  </div>
                </div>
                ...
                {generatedWorld && (
                  <div className="card p-6 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="text-lg font-semibold text-emerald-800">Generated World: {worldName}</h3>
                    </div>
                    <div className="prose prose-sm max-w-none">
                      <div className="whitespace-pre-wrap text-secondary-700">
                        {generatedWorld}
                    ...
                    <div className="mt-4 flex gap-2">
                      <button
                        onClick={() => navigator.clipboard.writeText(generatedWorld)}
                        className="btn-secondary text-sm"
                      >
                        <Copy className="w-4 h-4" />
                        Copy
      ...
      case 'stats':
        return <StatsTab userId={(session?.user as any)?.id} />
      
      case 'insights':
        return <InsightsTab userId={(session?.user as any)?.id} />
      
      case 'progress':
        return <ProgressTab userId={(session?.user as any)?.id} />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-100 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 py-8 relative z-10">
        ...
        {/* Tab Content */}
        <div className="tab-content transition-all duration-500 animate-fade-in-up relative z-10">
          {renderTabContent()}
        </div>
      </div>
    </div>
