...
**Tone:** Mystical and romantic`
  ...
  async generateMultipleStoryIdeas(genre: string = 'mixed', theme: string = 'creative'): Promise<string> {
    const contextInstruction = theme && theme !== 'creative' 
      ? `IMPORTANT: ALL story ideas MUST be directly related to the theme "${theme}". Every single idea should incorporate this theme as a central element.`
      : ''
    
    const genreInstruction = genre && genre !== 'mixed'
      ? `Focus on the ${genre} genre for all ideas.`
      : 'Include a variety of genres.'
    
    const prompt = `Generate 5 creative and unique story ideas. ${contextInstruction} ${genreInstruction} Each idea should be 1-2 sentences and include the genre. Format as a simple numbered list:

1. [Story idea 1 with genre]
2. [Story idea 2 with genre]
3. [Story idea 3 with genre]
4. [Story idea 4 with genre]
5. [Story idea 5 with genre]

Make each idea engaging, unique, and different from the others. ${contextInstruction}`

    try {
      const response = await this.generateWithGemini(prompt)
      return response.trim()
    } catch (error) {
      console.error('Error generating multiple story ideas:', error)
      return `1. A fantasy adventure about a young mage discovering their true power.
2. A sci-fi thriller set on a space station with mysterious disappearances.
3. A mystery novel about a detective solving crimes in a small town.
4. A romance story about two people from different worlds finding love.
5. A horror tale about a haunted house with a dark secret.`
    }
  }

  async generateCharacterFromType(characterType: string, additionalDetails: string = '', genre: string = 'fantasy'): Promise<string> {
    const contextInstruction = additionalDetails.trim() 
      ? `IMPORTANT: The character MUST be directly related to "${characterType}". Every aspect of the character should incorporate this type as a central element. Additional details: "${additionalDetails}"`
      : `IMPORTANT: The character MUST be directly related to "${characterType}". Every aspect of the character should incorporate this type as a central element.`
    
    const prompt = `Create a detailed character based on "${characterType}". ${contextInstruction}

Format as a clean, readable character profile (NO JSON). Include:
- Name and basic info
- Personality traits
- Background story
...
Make it engaging, unique, and directly related to ${characterType}. Write in a natural, storytelling style.`
    ...
    try {
      ...
      return `A ${characterType} character - A compelling and unique individual with their own story to tell.`
    }
  }

  async generatePlotOutline(genre: string, theme: string): Promise<string> {
    const prompt = `Create a detailed plot outline for a ${genre} story with the theme: ${theme}.
    ...
    *A ${genre} tale exploring ${theme}*
    ...
    1. **The Hook** - [Detailed description of the opening scene that grabs readers]
    2. **Setting the Stage** - [Detailed introduction of world, characters, and setup]
    3. **The First Challenge** - [Detailed description of the initial obstacle and conflict]
    4. **The Big Reveal** - [Detailed description of the major twist or discovery]
    5. **The Darkest Hour** - [Detailed description of the lowest point and crisis]
    6. **The Final Battle** - [Detailed description of the climactic confrontation]
    7. **New Beginnings** - [Detailed description of the resolution and character growth]

    Make each point feel like a natural part of the story's flow. Write with passion and insight, as if you're sharing the blueprint for an amazing story. Focus on emotional beats and character growth, not just plot mechanics. Provide specific, detailed content for each plot point that relates directly to the ${genre} genre and ${theme} theme.`
...
1. **The Hook** - Protagonist discovers a mysterious object that changes their world
2. **Setting the Stage** - Introduction to the world and key characters
3. **The First Challenge** - First major obstacle appears
4. **The Big Reveal** - A shocking revelation changes everything
5. **The Darkest Hour** - The darkest moment - all seems lost
6. **The Final Battle** - Final confrontation with the main antagonist
7. **New Beginnings** - Protagonist's growth and new world order`
    }
  }
...
Be specific and constructive in your analysis. Focus on what works well and provide actionable suggestions for improvement.`
  ...
  async generateChatResponse(userMessage: string, context?: string, systemMessage?: string): Promise<string> {
    const defaultSystemMessage = "You are a helpful AI assistant. Answer all questions directly and completely. When users ask you to write something (like a story, poem, dialogue, etc.), actually write it for them. When they ask questions, give direct answers. Be helpful, creative, and engaging."
    
    const systemPrompt = systemMessage || defaultSystemMessage
...
Please provide a helpful, direct response.`
    ...
    try {
      ...
      return "I'm here to help! Could you please rephrase your question? I'd be happy to assist you with whatever you need."
    }
  }
...
Return only the improved text, no explanations.`
  ...
  async generateWorldBuilding(context: string, genre: string, setting: string, additionalDetails: string = ''): Promise<string> {
    ...
    *A compelling tagline that captures the essence*
    ...
    Return this as a clean, readable world description (NO JSON). Focus on the narrative description above.
...
*A ${genre} world in ${setting}*

## **🌍 The World at a Glance**
This is a ${genre} world set in ${setting}, built around the concept of ${context}. The world features unique geography, cultures, and systems that make it feel alive and authentic.

## **⚡ How Magic Works Here**
The magic system in this world is designed around ${context}, with specific rules and limitations that create interesting conflicts and opportunities for storytelling.

## **👑 Power and Politics**
The political structure reflects the ${context} theme, with governments and power systems that feel natural to this world's unique characteristics.

## **🏛️ Daily Life and Culture**
The inhabitants of this world have developed cultures and societies that are deeply connected to ${context}, creating rich and diverse communities.

## **🗺️ The Land Itself**
The physical world is shaped by ${context}, with landscapes, climates, and natural features that support the world's unique identity.

## **📚 The Story So Far**
The world's history is built around ${context}, with ancient events and recent developments that give depth and meaning to the present.

## **👥 The People Who Matter**
The important figures and groups in this world are all connected to ${context}, creating compelling conflicts and relationships.

## **📜 The Rules That Matter**
This world operates by specific rules that make ${context} central to how everything works, creating a cohesive and believable setting.

## **⚔️ The Conflicts Brewing**
The tensions and conflicts in this world are all related to ${context}, creating natural sources of drama and storytelling opportunities.

## **🎭 The Heart of the Story**
The overall feeling of this world is shaped by ${context}, creating a unique atmosphere that supports the story you want to tell.`
    }
  }
